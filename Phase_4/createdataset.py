    ...
    if (end >= start):
        ...
        while (time <= end):
            ...
            if (image is not None):
                ...
                lakesMask = removeSea(ndwiMask)

                # get sea mask and make sure it is in the correct range 0-255
                seaMask = (np.ones(lakesMask.shape) * 255) - lakesMask - ndviMask
                seaMask = cv2.normalize(src = seaMask, dst = None, alpha = 0, beta = 255, norm_type = cv2.NORM_MINMAX, dtype = cv2.CV_8U)
...
def removeSea(msk):
    ...
    _, labelsIm = cv2.connectedComponents(msk)
    # find the labels that touch the border with the number of pixels that touch the border
    borderLabels = np.hstack([labelsIm[0, :], labelsIm[-1, :], labelsIm[:, 0], labelsIm[:, -1]])
    uniqueLabels, counts = np.unique(borderLabels, return_counts = True)
    # select only those labels that touch the border for at least 15% of the border
    borderLabels = uniqueLabels[counts >= (0.15 * msk.shape[0])]
